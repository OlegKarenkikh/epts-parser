...
#    csdo:Id50Type: нормализованная строка, 1..50 символов
...
#    Pattern: [A-F]|R](IV|I{1,3})?
_RE_CATEGORY = re.compile(r"^([A-FR](IV|I{1,3})?|R)$")
...
# 11.9. Год производства  →  bdt:YearType (ГОСТ ИСО 8601)
#    4-значное число, разумный диапазон 1900-2099
...
def validate_record(record: "VehiclePassportData") -> list[ValidationError]:
    ...
    def _check(field_name: str, pattern: re.Pattern, label: str) -> None:
        ...
        if raw is None:
            return  # optional field – absence is not an error
        value = str(raw).strip()
    ...
    # year — store as str, validate 4-digit range
    ...
    # eco_class — may be stored as Russian word or digit after parsing
